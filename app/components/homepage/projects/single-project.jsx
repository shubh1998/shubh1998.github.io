...
const SingleProject = ({ project }) => {
  const { name, description, tags = [project.demo], tools, code, demo, images = [] } = project;
  ...
  return (
    <div className='group w-full h-full flex flex-col items-center justify-center cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] shadow-2xl rounded-lg border border-[#1b2c68a0] relative bg-gradient-to-r from-[#0d1224] to-[#0a0d37]'
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        <h2 className='text-[#EFF3F4] not-italic font-semibold text-[1.525rem] leading-[110%] text-center capitalize'>
          <span className='text-pink-600'>{name.split(' - ')[0]}</span>
          &nbsp;-&nbsp;
          <span className='text-[#16EFB1]'>{name.split(' - ')[1]}</span>
        </h2>
        <hr className="mt-2 w-full h-px border-0 bg-gradient-to-r from-white/0 via-white/60 to-white/0 rounded-full" />
        <div className="p-6 w-full">
          <div className="w-full h-64 md:h-80">
            <ProjectCarousel 
              images={images.length ? images : ['/png/placeholder.png']} 
              name={name}
        ...
        <div className={`
          w-full gap-2 p-[0.825rem]
          rounded-[12px] border border-white/10
          bg-[radial-gradient(120%_120%_at_50%_10%,#ffffff0d_0%,#0a0d37_60%,#00000000_100%)]
          backdrop-blur-sm text-[#EFF3F4] flex flex-wrap
        `}>
          <span className="font-bold break-words text-sm">Tech Stack Used:</span>
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <Link
            href={demo}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#EFF3F4] text-[#EFF3F4] transition-all duration-300 hover:bg-[#231d4b] hover:text-violet-600 hover:border-[#0F0C41] hover:scale-110 cursor-pointer no-underline  delay-[0.3s] group-hover:translate-x-[-120px]"
          >
            <FaPlay />
          </Link>
        </div>
      </div>
      <p className="absolute w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[120%] rounded-[0_20px_20px_0] left-0 top-0 bg-[#0f0b24]  text-[#EFF3F4] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-base font-bold italic">
        {description}
      </p>
